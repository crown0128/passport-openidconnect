 ...
 * @param {Function} verify - callback to handle response from OIDC provider after sign in.
 *
 * @constructor
 * @public
 */
...
Strategy.prototype.authenticate = function (req, options) {
    ...
    if (req.query && req.query.code) {
        ...
        // allow for PKCE support
        var pkce = this._pkce;
        if (pkce) {
            var verifier = Buffer.from(crypto.pseudoRandomBytes(32)).toString(
                "base64"
            );
