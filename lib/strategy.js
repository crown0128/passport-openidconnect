 ...
 * @param {object | false} [user] - returns user object defined by the app or false if authentication fails at application.
 ...
 *
 * @callback skipLoadUserWithDone
 * @param {http.IncomingMessage} req
 * @param {*} claims
 * @param {(err: Error | null, skip:boolean) => void} [done]
 * @returns {void}
 */

/**
 * @callback skipLoadUser
 * @param {http.IncomingMessage} req
 * @param {*} claims
 * @returns {boolean}
 */
 ...
 * @param {boolean | http.Agent} [options.agent]
 ...
 * @param {string} [options.pkce] - defines which PKCE protocol to use. If not defined, PKCE is disabled.
 * @param {string} [options.sessionKey] - unqiue session id for this issuer. If none is given, issuer's hostname is used.
 * @param {SessionStore} [options.store] - custom session store instance
 * @param {skipLoadUserWithDone | skipLoadUser | boolean} [options.skipUserProfile] - determines if user data is loaded from /userInfo endpoint.
...
Strategy.prototype.authenticate = function (req, options) {
    ...
    if (req.query && req.query.code) {
                ...
                function (err, accessToken, refreshToken, params) {
                    ...
                    self._shouldLoadUserProfile(
                        ...
                        function (err, load) {
                            ...
                            // uiProfile == profile from /userInfo endpoint
                            function loaded(uiProfile, json, body) {
                                ...
                                var context = Context.parse(claims); // nothing from id token for this.
        ...
        var pkce = this._pkce;
        if (pkce) {
            var verifier = crypto.pseudoRandomBytes(32).toString("base64url");
